"""Hyperliquid VenueAdapter — wraps existing DirectHLProxy without rewriting it.

Converts between venue-agnostic Fill and HL-specific HLFill.
All real logic stays in cli/hl_adapter.py; this is a thin bridge.
"""
...
class HLVenueAdapter(VenueAdapter):
    """VenueAdapter implementation backed by DirectHLProxy.

    DirectHLProxy is already connected at construction time (it takes an HLProxy
    that has credentials), so connect() is a no-op.
    """

    def __init__(self, proxy: DirectHLProxy):
        self._proxy = proxy

    # --- Connection ---

    def connect(self, private_key: str, testnet: bool = True) -> None:
        """No-op — DirectHLProxy is already connected at init."""
        pass
    ...
    def set_leverage(self, leverage: int, coin: str, is_cross: bool = True) -> None:
        self._proxy.set_leverage(leverage, coin, is_cross)
